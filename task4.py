...
class Node:
    def __init__(self, key, color="#1296A0"):
        self.left = None
        self.right = None
        self.val = key
...
def build_heap_tree(heap, root, i, n):
    if i < n:
        root = Node(heap[i])

        root.left = build_heap_tree(heap, root.left, 2 * i + 1, n)
        root.right = build_heap_tree(heap, root.right, 2 * i + 2, n)
